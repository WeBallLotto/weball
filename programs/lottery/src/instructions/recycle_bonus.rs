...
pub fn handler(
    ...
    let now_ts = now_ts()?;
    let amount = ctx.accounts.draw.bonus_amount.try_sub(ctx.accounts.draw.paid_out_amount)?;
    if !ctx.accounts.draw.is_expired(now_ts)? || amount == 0 {
    ...
    pool.bonus_amount.try_sub_assign(amount)?;
    pool.prize_amount.try_add_assign(amount)?;

    // update draw
    let draw = &mut ctx.accounts.draw;
    draw.bonus_amount.try_sub_assign(amount)?;
