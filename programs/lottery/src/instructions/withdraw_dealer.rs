...
#[derive(Accounts)]
pub struct WithdrawDealer<'info> {
...
pub fn handler(
    ...
    if ctx.accounts.dealer.accrued_share_amount <= ctx.accounts.dealer.paid_out_amount {
        return Err(error!(ErrorCode::InsufficientBalance));
    }

    // transfer share amount
    let amount = ctx.accounts.dealer.accrued_share_amount.try_sub(ctx.accounts.dealer.paid_out_amount)?;
    token::transfer(ctx.accounts.transfer_ctx().with_signer(&[&ctx.accounts.pool.authority_seeds()]), amount)?;

    // update pool
    let pool = &mut ctx.accounts.pool;
    pool.dealer_paid_out_amount.try_add_assign(amount)?;
    // update dealer
    let dealer = &mut ctx.accounts.dealer;
    dealer.paid_out_amount.try_add_assign(amount)?;
